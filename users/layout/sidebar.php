<!--Start sidebar-wrapper-->
    <div id="sidebar-wrapper" data-simplebar="" data-simplebar-auto-hide="true">

        <div class="brand-logo">
            <a href="index.php">
                <!-- <img src="../assets/images/logo-icon.png" class="logo-icon" alt="logo icon"> -->
                <!-- <h5 class="logo-text">TEIPI CMS</h5> -->
                <img src="../assets/images/Tsukiden_Transparent_White.png" style="width:200px;" alt="logo icon">
            </a>
        </div><br>
        
        <ul class="sidebar-menu do-nicescrol">
            <li class="sidebar-header">MAIN NAVIGATION</li>
            <li>
                <a href="index.php">
                    <i class="fa-solid fa-dashboard"></i> <span>Dashboard</span>
                </a>
            </li>
            <li>
                <a href="employee_trans.php">
                    <i class="fa-solid fa-utensils"></i> <span>Canteen Transactions</span>
                </a>
            </li>
           <!-- <li>
                <a href="employee_hdm.php">
                    <i class="fa-solid fa-suitcase-medical"></i> <span>Health Declaration</span>
                </a>
            </li> -->
            <li>
                <a href="employee_attendance.php">
                    <i class="fa-solid  fa-address-book"></i> <span>Attendance</span>
                </a>
            </li>
            <li>
                <a href="downloads.php">
                    <i class="fa-solid  fa-download"></i> <span>Downloads</span>
                </a>
            </li>
            <!-- <li>
                <a href="#">
                    <i class="fa-solid fa-clock"></i> <span>Timekeeping</span>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="fa-solid fa-car-side"></i> <span>Shuttle Allocation</span>
                </a>
            </li> -->

            <li class="sidebar-header">SHORTCUTS</li>

            <li><a href="settings.php"><i class="fa-solid fa-cog"></i> <span>Settings</span></a></li>
            <li><a href="includes/logout.php"><i class="fa-solid fa-power-off text-danger"></i> <span>Log Out</span></a></li>
            <!-- <li><a href="javaScript:void();"><i class="fa-solid fa-triangle-exclamation text-warning"></i> <span>Warning</span></a></li>
            <li><a href="javaScript:void();"><i class="fa-solid fa-circle-info text-info"></i> <span>Information</span></a></li> -->

        </ul>
    
    </div>
<!--End sidebar-wrapper-->